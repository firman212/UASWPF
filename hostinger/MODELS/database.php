...
class database {
							...
							$namaayah,$namaibu,$alamatorangtua,$nilai)
	{
		...
		'$agama','$alamat','$tlp','$namasekolah','$namaayah','$namaibu','$alamatorangtua');");
	...
	function login($con,$user,$pass)
	{
		...
		if(!empty($cek[0]))
		{
			...
			header("location: olah.php?page=admin");
		}
		else
		{
			echo '
	...
	function hapusData($con,$kode)
	{
		...
		if($q1 & $q2)
		{
			...
			<script>
				...
				window.location.href="olah.php?page=admin";
			</script>
			';
		}
	}
	...
	function cek_pass($con, $user, $pass)
	{
	...
	function ganti_pass($con, $user, $pass)
	{
		$query = mysqli_query($con, "UPDATE tbl_admin SET password='$pass' WHERE username='$user'");
		if($query){
			echo "<script>window.alert('Ganti Password Berhasil !');
